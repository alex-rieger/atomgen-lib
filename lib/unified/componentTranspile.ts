  ...
  return function (ast: Root, { component }: ExtendedVFile) {
    ...
    visit(ast, "text", function (node) {
      ...
      if (tokenPrintBinding.matches(node)) {
        ...
        node.value = newValue;
        debug(
          `applying print binding handler`,
          "prop=",
          prop,
          "new value=",
          newValue
    ...
    visit(ast, "element", function (node) {
      if (tokenSlotBinding.matches(node)) {
        ...
        const propSerialized = Object.keys(node.properties!).find((p) =>
          p.startsWith("@show@")
        )!;
        const prop = tokenShowBinding.parse(propSerialized);
        const newValue = target.handler.show(node, prop, component);
        Object.assign(node, newValue);
        debug(`todo: applying show handler`, newValue);
      }
      if (node.properties) {
        ...
        Object.entries(node.properties).forEach(function (p) {
          ...
          if (val.some((v) => tokenPropBinding.matches(v))) {
            result = {
              [target.handler.prop.attr(attr, component)]: val.map((v) =>
                target.handler.prop.value(tokenPropBinding.parse(v), component)
              ),
            };
          } else if (val.some((v) => tokenClassNameBinding.matches(v))) {
            result = {
              [target.handler.className.attr(attr, component)]: val.map((v) =>
                target.handler.className.value(
                  tokenClassNameBinding.parse(v),
                  component
                )
              ),
            };
